...
from GIRAFFE.utils import interpolate_sphere
...
class Renderer(object):
    ...
    渲染方式包括:
        ...
        -- render_camera_elevation

        -- render_add_cars

    Params
    ------
    model -> nn.Module: 已训练的GIRAFFE模型
    ...
    def render_object_rotation(self, img_out_path, batch_size=15, n_steps=32):
        ...
        num_boxes = bbox_generator.num_boxes
        ...
        s_val = [[0, 0, 0] for i in range(num_boxes)]
        t_val = [[0.5, 0.5, 0.5] for i in range(num_boxes)]
        r_val = [0. for i in range(num_boxes)]
        s, t, _ = gen.get_transformations(s_val, t_val, r_val, batch_size)

        out = []
        for step in range(n_steps):
            # Get rotation for this step
            R = [step * 1.0 / (n_steps - 1) for i in range(num_boxes)]
    ...
    def render_object_translation_horizontal(self, img_out_path, batch_size=15,
        ...
        bg_rotation = gen.get_random_bg_rotation(batch_size)
        camera_matrices = gen.get_camera(batch_size=batch_size)
        num_boxes = gen.bounding_box_generator.num_boxes
        s = [[0., 0., 0.]
             for i in range(num_boxes)]
        r = [0.5 for i in range(num_boxes)]
    ...
    def render_interpolation(self, img_out_path, batch_size=15, n_samples=6,
        ...
        # Get values
        z_s_obj_1, z_a_obj_1, z_s_bg_1, z_a_bg_1 = \
        ...
        transformations = gen.get_transformations(
            [[0., 0., 0.] for i in range(num_boxes)],
            t_val,
            [0.5 for i in range(num_boxes)],
            batch_size
        )
        ...
        for j in range(n_samples):
            ...
            for step in range(n_steps):
                ...
                if mode == 'app':   # 以z_ii替代z_a_obj
                    latent_codes = [z_s_obj_1, z_ii, z_s_bg_1,
                                    z_a_bg_1]
                else:               # 以z_ii替代z_s_obj
                    latent_codes = [z_ii, z_a_obj_1, z_s_bg_1,
                                    z_a_bg_1]
                with torch.no_grad():
                    out_i = gen(batch_size, latent_codes, camera_matrices,
                                transformations, bg_rotation, mode='val')
    ...
    def render_interpolation_bg(self, img_out_path, batch_size=15, n_samples=6,
        ...
        for j in range(n_samples):
            ...
            for step in range(n_steps):
                w = step * 1.0 / ((n_steps) - 1)
                z_ii = interpolate_sphere(z_i1, z_i2, w)
                if mode == 'app':   # z_ii替换z_a_bg
                    latent_codes = [z_s_obj_1, z_a_obj_1, z_s_bg_1,
                                    z_ii]
                else:               # z_ii替换z_s_bg
                    latent_codes = [z_s_obj_1, z_a_obj_1, z_ii,
                                    z_a_bg_1]
                with torch.no_grad():
                    out_i = gen(batch_size, latent_codes, camera_matrices,
                                transformations, bg_rotation, mode='val')
    ...
    def render_object_translation_circle(self, img_out_path, batch_size=15,
        ...
        for step in range(n_steps):
            ...
            if num_boxes <= 2:
                t = [[0.5, 0.5, 0.] for i in range(num_boxes - 1)] + [
                    [cos_i, sin_i, 0]
                ]
    ...
    def render_camera_elevation(self, img_out_path, batch_size=15, n_steps=32):
        ...
        transformations = gen.get_transformations(
            [[0., 0., 0.] for i in range(num_boxes)],
            [[0.5, 0.5, 0.5] for i in range(num_boxes)],
            [0.5 for i in range(num_boxes)],
            batch_size,
        )
