...
def imread(filename):
    '''
    加载图片以numpy.ndarray形式，注意是通常的(height, weight, channels=3)的尺寸，忽略alpha通道
    '''
...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    if is_list:
        ...
        for i in tqdm(range(0, len(files), batch_size)):
            ...
            # 注意读取的图片ndarray.size=(height, weight, channels), dtype=np.uint8
